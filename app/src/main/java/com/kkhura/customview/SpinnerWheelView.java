 ...
 */
public class SpinnerWheelView extends View implements GestureDetector.OnGestureListener {
	...
	private float noOfLinesSeperated=5f;
	...
	public SpinnerWheelView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(attrs);
	}

	public SpinnerWheelView(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	protected void init(AttributeSet attrs) {
		...
		if (ta != null) {
			...
			noOfLinesSeperated=ta.getFloat(R.styleable.lwvWheelView_lwvNoOfLinesSeperate,noOfLinesSeperated);
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		//TODO to draw triangle comment below line
		...
		for (int i = start; i < end; i++) {
			float tempDis = mIntervalDis / noOfLinesSeperated;
			...
			for (int offset = -2; offset < 3; offset++) {
				...
				if (i >= 0 && i <= mMarkCount && mCenterIndex == i) {
					...
					if (tempOffset == 0) {
						// center highlighted line color
						mMarkPaint.setColor(mHighlightColor);
					}
					else if (tempOffset == 1)
					{
						//two lines on left and right of center line color
						mMarkPaint.setColor(mMarkColor);
					} else
					{
						//other lines color
				...
				{
					//Lines on the left hand side of center
					mMarkPaint.setColor(mMarkColor);
				}
				...
				if (offset == 0) {
					...
					//TODO to start line from top commet this line and uncomment line 2
			//		canvas.drawLine(ox, mTopSpace + smallMarkShrinkY, ox, mTopSpace + markHeight - smallMarkShrinkY, mMarkPaint);
					canvas.drawLine(ox, mTopSpace , ox, mTopSpace + markHeight - smallMarkShrinkY, mMarkPaint);
				}
			}
	...
	public interface OnWheelItemSelectedListener {
		void onWheelItemChanged(SpinnerWheelView wheelView, int position);

		void onWheelItemSelected(SpinnerWheelView wheelView, int position);
	}
	...
	static class SavedState extends BaseSavedState {
		...
		{
			return "SpinnerWheelView.SavedState{"
